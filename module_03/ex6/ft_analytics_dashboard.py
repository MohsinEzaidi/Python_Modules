...
def main() -> None:
    ...
    print('\n=== List Comprehension Examples ===')
    high_scorers = [name for name in data['players'] if data['players'][name]['total_score'] > 2000]
    print(f'High scorers (>2000): {high_scorers}')
    scores_doubled = [player['total_score'] * 2 for player in data['players'].values()]
    print(f'Scores doubled: {scores_doubled}')
    active_players = [name for name in data['players'] if data['players'][name]['sessions_played'] > 50]
    print(f'Active players: {active_players}')

    print('\n=== Dict Comprehension Examples ===')
    player_score = {player: score['total_score'] for player, score in data['players'].items()}
    print(f'Player scores: {player_score}')
    score_categories = {
        'high': len({p: s['total_score'] for p, s in data['players'].items() if s['total_score'] > 2000}),
        'medium': len({p: s['total_score'] for p, s in data['players'].items() if 1500 <= s['total_score'] <= 2000}),
        'low': len({p: s['total_score'] for p, s in data['players'].items() if s['total_score'] < 1500})
        }
    print(f'Score categories: {score_categories}')
    achievement_counts = {p: ac['achievements_count'] for p, ac in data['players'].items()}
    ...
    name = [player for player, info in data['players'].items() if info['total_score'] == max_score][0]
    print(f'Top performer: {name} ({data["players"][name]['total_score']} points, {data["players"][name]['achievements_count']} achievements)')
